...
class ClashIntegrationTests(unittest.TestCase):
    ...
    def test_clash_context(self) -> None:
        ...
        self.assertTrue(new_clash_context.detect_clash(exam2), "Expected programme level clash for same day, same course")
        new_clash_context.remove_rule("rule2")
        exam1: Exam = create_exam("COMP1601", date(2024,2,1), time(9, 0), time(11, 0))
        exam2: Exam = create_exam("COMP1602", date(2024,2,1), time(12, 0), time(14, 0))
        self.assertFalse(new_clash_context.detect_clash(exam2), "Expected no clash for same day after removing rule2")
        ...
        exam3: Exam = create_exam("COMP1603", date(2024,2,1), time(10, 0), time(12, 0))
        self.assertFalse(new_clash_context.detect_clash(exam3), "Expected no clash after removing rule1")
    ...
    def test_default_clash(self) -> None:
        ...
        self.assertFalse(default_clash.detect_clash(exam1), "Expected no clash for different level overlapping exams")
        create_exam("COMP1602", date(2024,2,2), time(9, 0), time(11, 0))
        self.assertFalse(default_clash.detect_clash(exam1), "Expected no clash for exams on different dates")
        create_exam("COMP1602", date(2024,2,1), time(10, 0), time(12, 0))
        self.assertTrue(default_clash.detect_clash(exam1), "Expected clash for same level overlapping exam times")
        create_exam("COMP1602", date(2024,2,1), time(9, 0), time(11, 0))
        self.assertTrue(default_clash.detect_clash(exam1), "Expected clash for exams at exactly the same time")
    ...
    def test_program_level_clash(self) -> None:
        ...
        exam1: Exam = create_exam("COMP1601", date(2024, 2, 1), time(9, 0), time(11, 0))
        self.assertFalse(program_clash.detect_clash(exam1), "Expected no clash for first exam")
        exam2: Exam = create_exam("COMP1602", date(2024, 2, 1), time(14, 0), time(16, 0))
        self.assertTrue(program_clash.detect_clash(exam2), "Expected clash for same day, same programme and level")
        exam3: Exam = create_exam("COMP1602", date(2024, 2, 2), time(9, 0), time(11, 0))
        self.assertFalse(program_clash.detect_clash(exam3), "Expected no clash for different day")
