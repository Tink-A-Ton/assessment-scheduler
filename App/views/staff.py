...
from App.controllers import CourseInstructor
...
from App.controllers.staff import get_registered_courses

from App.controllers.course import list_Courses
...
def detect_clash(id):
    ...
    all_assessments = CourseAssessment.query.filter(
        not_(CourseAssessment.a_ID.in_([2, 4, 8]))
    ).all()
...
def get_account_page():
    ...
    return render_template(
        "account.html", courses=courses, registered=registered_courses
    )
...
def get_assessments_page():
    ...
    return render_template(
        "assessments.html", courses=registered_courses, assessments=assessments
    )
...
def get_add_assessments_page():
    ...
    return render_template(
        "addAssessment.html", courses=registered_courses, assessments=allAsm
    )
...
def add_assessments_action():
    ...
    newAsm = add_CourseAsm(
        course, asmType, startDate, endDate, startTime, endTime, False
    )
    if newAsm.startDate:
